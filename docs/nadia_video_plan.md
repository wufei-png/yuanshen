...
> 状态：V01/V03 两条 720p 试片已生成且视觉检查通过；V04/V05 两条 720p 试片已导入并完成文件与抽帧核验；V07/V08 已从 Downloads 导入并完成文件与抽帧核验；六条当前 720p 视频已复制到公开 HTML 资源目录并完成挂载。V06 视频已放弃，K10/K11 仅保留为静态动作板参考。**2026-09-04：六条 720p 技术/音频验收全部通过（见执行单 §8）**；1080p 定稿与叙事 PV 的执行计划见 §11。
> 角色：娜蒂娅「两衡之间」
...
| 项       | 选择                                                                                                         |
| -------- | ------------------------------------------------------------------------------------------------------------ |
| 去处     | HTML 分享包的 720p 预览与 PV 共用同一套 720p 成片；**2026-09-04 决策：720p 直接定稿，取消 1080p**      |
| 模型     | Seedance 2.0；试片优先标准模型（原 1080p 定稿不用 Fast 的限制随 1080p 取消而失效）                                      |
...
| 视频试跑 | V01/V03 各 6s、V04/V05 各 5s、V07 约 8s、V08 约 6s 的 720p 已导入；2026-09-04 技术/音频验收全部通过（执行单 §8）；**同日本轮决策：720p 为定稿档，不再制作 1080p** |
...
| C    | V01、V03–V05 试片来自 `tmp/video_trials/`，V07/V08 来自 Downloads；当前副本已进入公开资源 | **已完成/验收通过（2026-09-04）** |
| D    | ~~1080p 定稿~~ 已取消：720p 即定稿（2026-09-04 决策）                                | 不执行             |
| E    | 叙事 PV 混音剪辑（720p）；HTML 挂载                                                     | **已完成（2026-09-04）** |
...
| watermark      | V01、V03–V05 均实测有“AI生成”；V07/V08 保留源文件状态，仍如实记录提交页状态。2026-09-04 验收补充：V08 抽帧检出“AI生成”，V07 多时刻抽帧未检出、保留源文件状态（见执行单 §8） |
...
成片文件：`output/nadia_character/assets/video/nadia_video_pv_between_two_weights_v1.mp4`，16:9 **720p**（2026-09-04 决策与六条试片同档，取消 1080p）。时间轴：
...
| 0:26–0:31  | V04                       | 07「普莎，顺着风——慢一点。」                             |
| 0:31–0:36  | V05                       | 08「伊嘉，稳住落点。」                                   |
...
| 镜头     | 建议位置                                                                            |
| -------- | ----------------------------------------------------------------------------------- |
| V01      | `index.html` 首屏保留 splash 静态主视觉；首屏下方独立介绍影像区展示 V01，K01 作 poster |
| V07、V08 | `index.html` `#companions` 下方的成对“生活片段”区                                |
| V03–V05  | `skills.html` `#boards` 的 BOARD 02–04：静态主图与视频动作预览依次呈现                |
| V02、V06  | 不挂视频；BOARD 01、06 保留静态动作板；BOARD 05 本批不做视频                   |
| PV       | `index.html` `#intro-motion` 区 V01 卡下方（2026-09-04 已挂载，poster 用 K01）           |
...
8. 生成音轨没有中文或英文句子。
9. 720p 试片通过后即定稿（2026-09-04 决策取消 1080p）；PV 与 HTML 共用 720p 成片。
...
执行门：V04/V05/V07/V08 先完成音频、时长、画幅、编码和画面连续性复核，并记录可见水印状态；当前六条视频已经进入 HTML 预览，不因水印单独触发重跑。**2026-09-04 六条 720p 验收全部通过（执行单 §8），并决策：720p 直接定稿，取消 1080p；叙事 PV 按 720p 制作（§11 执行记录）。**

执行顺序（已完成）：

1. 完成 V01/V03/V04/V05/V07/V08 的音频、界面参数和 720p 技术验收（2026-09-04，执行单 §8）。
...
5. ~~复制通过项生成 1080p~~ 已取消（2026-09-04 决策）；720p 公开副本即定稿，PV 与 HTML 共用同一套 720p 成片。

---

## 11. 720p 定稿与叙事 PV 执行记录（2026-09-04）

> 前置：六条 720p 已于 2026-09-04 全部 `pass`（[`nadia_video_pilot.md`](nadia_video_pilot.md) §8）。**同日本轮决策：720p 直接定稿，取消 1080p 重生成；叙事 PV 按 720p 制作并完成挂载。**以下为执行记录，取代原 1080p 任务计划。

### 11.1 决策与边界

1. 720p 即定稿档：六条公开副本与 PV、HTML 共用同一套 720p 成片；不再调用外部生成服务。
2. V06 始终不进入视频与 PV；K10/K11 仅保留静态动作板参考。
3. 关键帧全部复用已视觉确认的 1920×1080 PNG（K01、K01E、K04 v3、K05 v3、K06、K07、K08 v3、K09 v9、K12、K13 v4、K14、K15），不重出。
4. 水印沿用用户既定接受口径，不因水印重跑。

### 11.2 PV 混音执行记录

- 成片：`output/nadia_character/assets/video/nadia_video_pv_between_two_weights_v1.mp4`，16:9 **720p**，42.000s，H.264/yuv420p/24fps + AAC 48kHz，19.7MB。
- 时间轴（§6）：V01(0:00–0:06) → V07(0:06–0:14) → V08(0:14–0:20) → V03(0:20–0:26) → V04(0:26–0:31) → V05(0:31–0:36) → 回切 V01 末拍静帧 + 风床收尾(0:36–0:42)，最后 1.5s 淡出至黑。
- 对白叠轨入点（clip 内）：V01 0.35s、V07 0.35s、V08 1.5s、V03 1.55s、V04 3.58s（主响应闪光前 0.02s）、V05 3.18s（落点前 0.02s）。v3.1 重录后，V04 新语音约 2.86s，仍延伸进 V05；V05 新语音约 1.83s，在片尾前结束。
- ducking：对白窗口内环境轨约 -10dB（6 个窗口：0.35–5.53 / 6.43–11.48 / 15.68–20.31 / 21.81–25.10 / 29.92–32.85 / 34.60–36.55）。
- 尾段风床：取 V01 音轨 0.9–6.08s，延时至 36.544s 起播，0.6s 淡入；片尾字幕「娜蒂娅「两衡之间」」由 Pillow 渲染透明 PNG（STHeiti、暖金、深色描边），39.4s 起 1.2s 淡入，随全局淡出。
- 响度：整片 loudnorm 至 I=-16 LUFS（实测 -16.9），TP 限幅 0.95。
- 工具：ffmpeg 8.1 本地完成（concat → volume 时间窗 ducking → amix 对白 → loudnorm → 淡出）。v3.1 使用 `python3 tools/remix_nadia_pv.py` 从六条原始试片和六条独立语音重建音轨，保留原 PV 视频流。

### 11.3 PV 验收结果（2026-09-04 画面基线；2026-09-21 语音重混）

- 全解码无错；时长 42.000s；1280×720/16:9；H.264/yuv420p/24fps + AAC 48kHz stereo。
- 硬切 6 处，全部落在片段边界（6.13 / 14.21 / 20.29 / 26.38 / 31.46 / 36.50），无意外剪辑点。
- 对白窗口音频实测：六窗 mean_volume -18.2～-20.2 dB（可懂度正常）；尾段 38–39s 风床约 -38 dB；41s 淡出中。
- 片尾字幕 t=40.3 OCR 检出「娜蒂娅「两衡之间」」；t=41.9 帧亮度 10/255（淡出至黑）。
- v3.1 保留原视频流，使用新 07/08 语音重混；全解码无错，42.000s，1280×720/24fps + AAC 48kHz stereo，整体 -16.9 LUFS，真峰值 -1.0 dBFS。语音识别复核 PV 两段对应“顺着风慢一点”“稳住落点”，旧术语台词不再出现在音轨。
- 已知近似：V04/V05 对白入点按提示词动作节拍推定，未做逐帧人耳听感校准；如需精调，只改 `tools/remix_nadia_pv.py` 的入点时间并重渲染即可。

### 11.4 HTML 挂载与同步

- `index.html` `#intro-motion` 区 V01 卡下方新增叙事短片卡（`controls`/`playsinline`/`preload="none"`，poster 用 K01）；导航「介绍影像」入口不变。
- 分享 ZIP 已重打包并 `unzip -tq` 核验；同步文件见提交记录。

### 11.5 明确不做

V02（只保留技能页静态板）、V06（视频已放弃）、BOARD 05 归衡校读、十二秒场地循环、竖屏、Q 视频、料理、生日、1080p（已取消）。
