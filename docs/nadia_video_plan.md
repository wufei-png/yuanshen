...
- 对白叠轨入点（clip 内）：V01 0.35s、V07 0.35s、V08 1.5s、V03 1.55s、V04 3.58s（主响应闪光前 0.02s）、V05 3.18s（落点前 0.02s）；对白时长超出所在片段的部分自然延续到下一片段（V04 语音延伸 1.2s 进 V05、V05 语音延伸进尾段），属于有意编排。
- ducking：对白窗口内环境轨 -10dB（volume 时间窗自动化，6 个窗口：0.35–5.53 / 6.43–11.48 / 15.68–20.31 / 21.81–25.10 / 29.92–32.64 / 34.60–37.69）。
...
- 工具：ffmpeg 8.1 本地完成（concat → volume 时间窗 ducking → amix 对白 → loudnorm → 淡出）。

### 11.3 PV 验收结果（2026-09-04）
...
- 已知近似：V04/V05 对白入点按提示词动作节拍推定，未做逐帧听感校准；如需精调，只改 §11.2 的入点时间并重渲染即可。
