...
| 视频试跑 | V01/V03 各 6s、V04/V05 各 5s、V07 约 8s、V08 约 6s 的 720p 已导入；保留源文件编码/水印状态；2026-09-04 技术/音频验收全部通过（执行单 §8） |
...
| D    | 通过验收的镜头制作 1080p 定稿 → `output/nadia_character/assets/video/`          | 试片已通过；执行计划见 §11，待提交生成任务 |
| E    | 叙事 PV 混音剪辑；HTML 挂载                                                     | HTML 已挂载（720p）；PV 混音见 §11，待 1080p 定稿后执行 |
...
## 11. 1080p 定稿与叙事 PV 执行计划（2026-09-04 更新）

> 前置：六条 720p 已于 2026-09-04 全部 `pass`（[`nadia_video_pilot.md`](nadia_video_pilot.md) §8）。本计划把 §10.1 的“下一步”落成可执行任务；**1080p 结果仍需独立重新验收，不能由 720p 通过自动推导**。

### 11.1 执行门

1. 只复制已通过验收的提示词版本与关键帧；V06 始终不进入视频与 PV。
2. 关键帧全部复用已视觉确认的 1920×1080 PNG（K01、K01E、K04 v3、K05 v3、K06、K07、K08 v3、K09 v9、K12、K13 v4、K14、K15），不再重出。
3. 1080p 每次生成都新建 `attempt_id: <id>_1080p_a01` 记录并写入执行单 §7 模板；重跑追加 `a02`，不覆盖。
4. 任一身份、连续性、动作落点或音轨门失败，回到 720p 提示词/关键帧调整，不硬烧 1080p。
5. 模型优先 Seedance 2.0 标准模型，定稿不用 Fast（若入口提供该变体）；实际入口名称与界面开关如实记录。

### 11.2 1080p 任务表

| 任务 | 关键帧（复用） | 提示词版本（复制自） | 时长 | 输出文件名 |
| --- | --- | --- | --- | --- |
| V01 登场 | K01 + K01E | `nadia_video_pilot_v5` | 6s | `nadia_video_v01_intro_1080p_a01.mp4` |
| V03 战技 | K04 v3 + K05 v3 | `nadia_video_pilot_v6` | 6s | `nadia_video_v03_skill_1080p_a01.mp4` |
| V04 轻相 | K06 + K07 | `nadia_video_pilot_v14` | 5s | `nadia_video_v04_light_1080p_a01.mp4` |
| V05 重相 | K08 v3 + K09 v9 | `nadia_video_pilot_v17` | 5s | `nadia_video_v05_heavy_1080p_a01.mp4` |
| V07 风 | K12 + K13 v4 | `nadia_video_pilot_v23d` | 8s | `nadia_video_v07_wind_1080p_a01.mp4` |
| V08 抱错了 | K14 + K15 | `nadia_video_pilot_v24` | 6s | `nadia_video_v08_hug_1080p_a01.mp4` |

共用参数：`16:9 / 1920×1080 / generate_audio=true`，声音只允许环境音与动作音效、禁止对白；V01 追加执行单 §4.1 公共尾句，V04/V05/V07/V08 不追加；镜头运动按对应执行单章节。水印开关按界面实际状态记录，不因水印单独重跑（沿用用户既定接受口径）。

### 11.3 1080p 验收门

沿用执行单 §6.2 全部门，另加：

1. 分辨率实测 `1920×1080`、H.264 或平台输出等价编码、音轨存在且非静音（EBU R128 整体响度记录）。
2. 抽首/中/尾帧核对身份、构图与动作落点；SSIM 变化趋势与对应镜头语义一致且无硬切（scene>0.35 计数为 0）。
3. 对白不入画面声轨；后期叠轨按 §5.2 与 11.4 执行。
4. 完成后按执行单 §7 模板写入 `attempt_id` 记录；每条通过后才允许进入 PV 混音。

### 11.4 叙事 PV 混音剪辑

- 成片：`output/nadia_character/assets/video/nadia_video_pv_between_two_weights_v1.mp4`，16:9 1080p，约 42s。
- 时间轴沿用 §6：V01(0:00–0:06) → V07(0:06–0:14) → V08(0:14–0:20) → V03(0:20–0:26) → V04(0:26–0:31) → V05(0:31–0:36) → 回切 V01 末拍静帧收尾(0:36–0:42)。
- 对白叠轨与入点按执行单 §5 表（V01 0.35s、V07 0.35s、V08 1.5s、V03 1.55s、V04/V05 主响应前约 0.02s）；生成轨 ducking 8–12 dB；V01 生成轨先做约 +8～12 dB 增益再混音（§8 响度结论）。
- 转场：硬切或 8–12 帧叠化；不加标题卡字幕；片尾如需文字，后期单独排「娜蒂娅「两衡之间」」，不用 Seedance 烧字。
- 混音工具：ffmpeg 本地完成（`adelay`/`amix`/`sidechaincompress` 或等效），完成后做一次整片响度与对白可懂度抽查。
- HTML：`index.html` 导航新增「介绍影像」或档案页底部一节挂 PV；`<video>` 用 `controls`、`playsinline`、`preload="none"`、poster 指向 V01 关键帧；不进明信片页。

### 11.5 执行顺序与分工

1. 用户提交六条 1080p 生成任务（涉及外部生成服务调用）。
2. 生成结果落地后，由我按 §11.3 做客观技术复核（SHA/编码/分辨率/响度/硬切/SSIM/水印 OCR）。
3. 用户做视觉与听感终审；通过后逐条写入执行单 §7 记录并更新 §8 结论。
4. 六条 1080p 全部通过后执行 PV 混音（§11.4），输出 v1 成片。
5. HTML 挂载 PV、替换/保留 720p 预览策略按 §7；最终重打包分享 ZIP 并同步 README。

### 11.6 明确不做

V02（只保留技能页静态板）、V06（视频已放弃）、BOARD 05 归衡校读、十二秒场地循环、竖屏、Q 视频、料理、生日。
